...
def fetch_data(stock):
    ...
    print(f"Fetching data for {stock} from 2015-04-01 to {end_date}")
    data = yf.download(stock, start="2015-04-01", end=end_date, interval="1d")
    if data.empty:
        print(f"No data returned for {stock}. DataFrame is empty.")
        return None

    print(f"Data fetched for {stock}: {data.shape} rows")
...
for stock in stocks:
    ...
    if result is None:
        print(f"Failed to fetch data for {stock}, skipping...")
        continue
...
# Start the dashboard even if no data is fetched
if not data_dict:
    print("No stock data fetched. Starting dashboard with limited functionality.")
else:
    print(f"Starting dashboard with data for {list(data_dict.keys())}")
...
def update_dashboard(selected_stock, n_intervals):
    ...
    if result is None:
        return html.Div(f"Error fetching data for {selected_stock}.", style={"color": "#ffffff"})
    data, news_df, nsmi_df = result

    # Update dictionaries
    data_dict[selected_stock] = data
    news_dict[selected_stock] = news_df
    nsmi_dict[selected_stock] = nsmi_df
    model = model_dict.get(selected_stock)
